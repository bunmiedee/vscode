...
export namespace ChatContextKeys {
	...
	/**
	 * True when the chat input is rendered inside the swarm agent grid. In this
	 * surface the composer is restructured: the secondary toolbar row is removed
	 * and the permission picker plus the Pair picker move into the textfield
	 * toolbar.
	 */
	export const swarmComposer = new RawContextKey<boolean>('chatSwarmComposer', false, { type: 'boolean', description: localize('swarmComposer', "True when the chat input is rendered inside the swarm agent grid.") });
